...
def getDataFromTxt(txt, with_landmark=True,flag = 0):
    """
    ...
    for line in lines:
        ...
        #if flag ==1:
        img_path = components[0] # file path
        #else:
        ...
        if ourcameraflag:
            ...
            landmarks = np.array(components[1:11], dtype=np.float32).reshape(-1, 10)
            #print(landmarks)
            ...
            for index in range(0, 5):
                rv = (float(components[1+2*index]), float(components[1+2*index+1]))
                landmark[index] = rv
