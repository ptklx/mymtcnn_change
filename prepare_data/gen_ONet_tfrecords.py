...
readtxt = './imglists/OnetA/NIR_calib_A/train_Net_neg_single.txt'
selectFlag = 0 #  0 negative  1 part  2 positive  3 landmark
output_directory = './imglists/OnetA/%s'%Mark
...
def _add_to_tfrecord(filename, image_example, tfrecord_writer):
    ...
    image_data, height, width = _process_image_withoutcoder(filename,COLOR_GRAY,48)
...
def _get_output_filename(output_dir, name, net):
    ...
    if selectFlag == 0:
        return '%s/ONet_48_%s_neg_single.tfrecord' % (output_dir,grayColor)
    elif selectFlag ==1:
        return '%s/ONet_48_%s_part.tfrecord' % (output_dir,grayColor)
    elif selectFlag ==2:
        return '%s/ONet_48_%s_pos.tfrecord' % (output_dir,grayColor)
    elif selectFlag ==3:
        return '%s/ONet_48_%s_landmark.tfrecord' % (output_dir,grayColor)
