...
def _process_image_withoutcoder(filename,flag,size = 12):
    assert os.path.exists(filename)
    if flag ==1:   #gray
        image = cv2.imread(filename,cv2.IMREAD_GRAYSCALE)   #   notice  
        ...
        image_data = image.tostring()
        assert len(image.shape) == 2   # 3
        height = image.shape[0]
        width = image.shape[1]
        #assert image.shape[2] == 3
        #assert image.shape[2] == 1
    elif flag ==3:  #color
        #image = cv2.imread(filename)   #   notice  
