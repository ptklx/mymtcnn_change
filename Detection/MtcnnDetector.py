...
class MtcnnDetector(object):
    ...
    def detect_rnet(self, im, dets):
        ...
        if self.COLO_GRAY == 0:
            ...
            for i in range(num_boxes):
                ...
                cropped_ims[i, :, :, :] = (cv2.resize(tmp, (24, 24))-127.5) / float(128)
      
        elif self.COLO_GRAY ==1:
            cropped_ims = np.zeros((num_boxes, 24, 24, 1), dtype=np.float32)
            for i in range(num_boxes):
                tmp = np.zeros((tmph[i], tmpw[i], 1), dtype=np.uint8)
                tmp[dy[i]:edy[i] + 1, dx[i]:edx[i] + 1,0] = im[y[i]:ey[i] + 1, x[i]:ex[i] + 1]
                cropped_ims[i, :, :, 0] = (cv2.resize(tmp, (24, 24))-127.5) / float(128)
    ...
    def detect(self, img):
        ...
        if self.pnet_detector:
            ...
            t1 = time.time() - t
            print(" time cost  pnet {:.3f} ".format(t1))
            t = time.time()
        # rnet
        t2 = 0
        if self.rnet_detector:
            ...
            t2 = time.time() - t
            print(" time cost  rnet {:.3f} ".format(t2))
            t = time.time()
        # onet
        t3 = 0
    ...
    def detect_new(self, img):
        ...
        t3 = 0
        if self.onet_detector:
            #cls_scores = self.detect_onet(img, boxes_c)
            dets = boxes_c
            im = img
            if len(im.shape) == 3:
                h, w, c = im.shape
            ...
            if self.COLO_GRAY == 0:
                ...
                for i in range(num_boxes):
                    ...
                    tmp[dy[i]:edy[i] + 1, dx[i]:edx[i] + 1,0] = im[y[i]:ey[i] + 1, x[i]:ex[i] + 1]
                    cropped_ims[i, :, :, 0] = (cv2.resize(tmp, (48, 48))-127.5) / float(128)
            cls_scores = self.onet_detector.predict(cropped_ims)
            ...
            print(
                "time cost " + '{:.3f}'.format(t1 + t2 + t3) + '  pnet {:.3f}  rnet {:.3f}  onet {:.3f}'.format(t1, t2,
                                                                                                                t3))
            '''
        return cls_scores,all_boxes,flag

    def detect_facePnet(self, test_data):
    ...
    def detect_face(self, test_data):
        ...
        for databatch in test_data:
            ...
            if self.pnet_detector:
                ...
                boxes, boxes_c, landmark = self.detect_pnet(im)
                t1 = time.time() - t
                if not self.onet_detector  or not self.rnet_detector:
                    print(" time cost  pnet {:.3f} ".format(t1))
                sum_time += t1
                if boxes_c is None:
                    #print("boxes_c is None...")
