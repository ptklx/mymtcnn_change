...
def P_Net_new(inputs,label=None,bbox_target=None,training=True):
    ...
    with slim.arg_scope([slim.convolution2d, slim.separable_convolution2d],
        ...
        with slim.arg_scope([slim.batch_norm],
            ...
            print (net.get_shape())
            net = _depthwise_separable_conv(net, 12, 1, downsample=True, sc='down_1')
            #net = slim.max_pool2d(net, kernel_size=[2,2], stride=2, scope='pool1', padding='SAME')
            print (net.get_shape())
            net = _depthwise_separable_conv(net, 16, 1, sc='conv3')
            net = _depthwise_separable_conv(net, 16, 1, downsample=True, sc='down_2')
            print (net.get_shape())
            net = _depthwise_separable_conv(net, 32, 1, sc='conv4')
            net = _depthwise_separable_conv(net, 32, 1, downsample=True, sc='down_3')
            net = _depthwise_separable_conv(net, 32, 1, downsample=True, sc='down_4')
            print (net.get_shape())
            ...
            conv6 = slim.conv2d(net,num_outputs=2,kernel_size=[1,1],stride=1,scope='conv4_1',activation_fn=tf.nn.softmax)
